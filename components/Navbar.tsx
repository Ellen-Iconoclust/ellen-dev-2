...
const Navbar: React.FC<NavbarProps> = ({ onNavigate }) => {
  ...
  return (
    <>
      <nav className="fixed top-8 left-0 w-full z-[100] flex justify-center pointer-events-none transition-all duration-500">
        <div className={`pointer-events-auto group bg-white/5 backdrop-blur-xl border border-white/10 rounded-full px-6 py-3 flex items-center gap-6 transition-all duration-500 max-w-[90vw] ${isScrolled ? 'py-2 px-4 bg-white/10' : ''}`}>
          ...
          <button 
            ...
            className="font-black text-xl tracking-tighter flex items-center gap-2 text-white outline-none"
          >
            ELLEN<span className="font-premium-italic text-sm">©</span>
          </button>

          <div className={`hidden md:flex gap-8 overflow-hidden transition-all duration-500 ${isScrolled ? 'w-0 opacity-0 pointer-events-none' : 'w-auto opacity-100'}`}>
          ...
          <button 
            className="md:hidden text-white p-1 hover:bg-white/10 rounded-full transition-colors outline-none"
      ...
      <div className={`fixed inset-0 bg-black/60 backdrop-blur-sm z-[105] md:hidden transition-opacity duration-300 ${isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMenuOpen(false)} />
